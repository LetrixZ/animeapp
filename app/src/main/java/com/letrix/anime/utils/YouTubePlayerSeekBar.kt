...
class YouTubePlayerSeekBar(context: Context, attrs: AttributeSet? = null) : LinearLayout(context, attrs), SeekBar.OnSeekBarChangeListener, YouTubePlayerListener {
    ...
    init {
        ...
        val showVideoDuration = typedArray.getBoolean(R.styleable.YouTubePlayerSeekBar_showDuration, true)
        val showCurrentTime = typedArray.getBoolean(R.styleable.YouTubePlayerSeekBar_showCurrentTime, true)
        val showThumb = typedArray.getBoolean(R.styleable.YouTubePlayerSeekBar_showThumb, true)
        val seekFullWidth = typedArray.getBoolean(R.styleable.YouTubePlayerSeekBar_seekFullWidth, false)
        val customSeekBar = typedArray.getResourceId(R.styleable.YouTubePlayerSeekBar_customSeekBar, 0)
        val customThumb = typedArray.getResourceId(R.styleable.YouTubePlayerSeekBar_customSeekBar, 0)
        val seekable = typedArray.getBoolean(R.styleable.YouTubePlayerSeekBar_seakable, true)
        ...
        if (!seekFullWidth) seekBar.setPadding(padding * 2, padding, padding * 2, padding)
        else seekBar.setPadding(0, 0, 0, 0)

        setColor(color)

        if (customSeekBar != 0) {
            seekBar.progressDrawable = ContextCompat.getDrawable(context, customSeekBar)
        }

        if (customThumb != 0) {
            seekBar.thumb = ContextCompat.getDrawable(context, customThumb)
        }
...
//        seekBar.secondaryProgress = 70
    ...
    fun showCurrentTime(show: Boolean) {
        videoCurrentTimeTextView.visibility = if (show) View.VISIBLE else View.GONE
    }

    fun showVideoDuration(show: Boolean) {
        videoDurationTextView.visibility = if (show) View.VISIBLE else View.GONE
    }

    fun showThumb(show: Boolean) {
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
            seekBar.splitTrack = show
        }
        seekBar.thumb.mutate().alpha = if (show) 255 else 0
    }
